...
export class UserProfileComponent {
  ...
  async loadGames() {
    const response = await this.userService.getGamesHistory(
      this.savePagination()
    );
  ...
  async checkFriendship() {
    ...
    this.isFriend =
      this.friendService.connectedFriends.some(
        (friend) => friend.id === this.profileId
      ) ||
      this.friendService.disconnectedFriends.some(
        (friend) => friend.id === this.profileId
      );
